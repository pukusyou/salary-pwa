import React from "react";

interface Drink {
  name: string;
  price: number;
  value: number;
}

const AddRemoveList = ({
  index,
  drink,
  setDrink,
  handleRemoveDrink,
}: {
  index: number;
  drink: Drink;
  setDrink: Function;
  handleRemoveDrink: Function;
}) => {
  const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setDrink(index, { ...drink, price: Number(e.target.value) });
  };

  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setDrink(index, { ...drink, name: e.target.value });
  };

  const handleValueChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setDrink(index, { ...drink, value: Number(e.target.value) });
  };

  return (
    <div
      key={drink.name}
      className="flex items-center space-x-4 mb-4 bg-white rounded-lg shadow-md p-4 "
    >
      <div className="flex flex-col flex-1 w-1/3">
        <label
          htmlFor={`drinkName-${drink.name}`}
          className="text-sm font-medium text-gray-700"
        >
          名称
        </label>
        <input
          type="text"
          id={`drinkName-${drink.name}`}
          placeholder="名称"
          value={drink.name}
          onChange={handleNameChange}
          className="border rounded px-3 py-2 mt-1"
        />
      </div>
      <div className="flex flex-col flex-1 w-1/3">
        <label
          htmlFor={`drinkPrice-${drink.name}`}
          className="text-sm font-medium text-gray-700"
        >
          値段
        </label>
        <input
          type="number"
          id={`drinkPrice-${drink.name}`}
          placeholder="値段"
          value={drink.price}
          onChange={handlePriceChange}
          className="border rounded px-3 py-2 mt-1"
        />
      </div>
      <div className="flex flex-col flex-1 w-1/4">
        <label
          htmlFor={`drinkPrice-${drink.name}`}
          className="text-sm font-medium text-gray-700"
        >
          個数
        </label>
        <input
          type="number"
          id={`drinkPrice-${drink.name}`}
          placeholder="個数"
          value={drink.price}
          onChange={handleValueChange}
          className="border rounded px-3 py-2 mt-1"
        />
      </div>
      <button
        onClick={() => handleRemoveDrink(drink.name)}
        className="border rounded-full px-2 py-2 bg-red-500 text-white flex items-center justify-center"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="w-6 h-6"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
          />
        </svg>
      </button>
    </div>
  );
};

export default AddRemoveList;
